...
%inline
public export
pure : ({s : state} -> a s) -> Parser state tok True locked free a
pure x = x <$ Seq []
...
(<**>) :
  {nil1, nil2 : Bool} ->
  Parser state tok nil1 locked free a ->
  Parser state tok nil2 (linUnless nil1 locked) (free ++ linUnless (not nil1) locked) b ->
  Parser state tok (nil1 && nil2) locked free (\s => (a s, b s))
...
(<*>) :
  {nil1, nil2 : Bool} ->
  Parser state tok nil1 locked free (\s => a s -> b s) ->
  Parser state tok nil2 (linUnless nil1 locked) (free ++ linUnless (not nil1) locked) a ->
  Parser state tok (nil1 && nil2) locked free b
...
(<*) :
  ...
  Parser state tok nil1 locked free a ->
  Parser state tok nil2 (linUnless nil1 locked) (free ++ linUnless (not nil1) locked) b ->
  Parser state tok (nil1 && nil2) locked free a
p <* q = fst <$> (p <**> q)
...
(<|>) :
  {nil1, nil2 : Bool} ->
  Parser state tok nil1 locked free a ->
  Parser state tok nil2 locked free a ->
...
(<||>) :
  {nil1, nil2 : Bool} ->
  Parser state tok nil1 locked free a ->
  Parser state tok nil2 locked free b ->
  {auto 0 prf : length (filter Basics.id [nil1, nil2]) `LTE` 1} ->
  Parser state tok (nil1 || nil2) locked free (\s => Either (a s) (b s))
p <||> q = Left <$> p <|> Right <$> q

%inline
public export
match : TokenKind tok => (kind : tok) -> Parser state tok False locked free (const $ TokType kind)
...
enclose :
  {b1, b2, b3 : Bool} ->
  (left : Parser state tok b1 locked free (const ())) ->
  (right :
    Parser state tok b3
      (linUnless b2 (linUnless b1 locked))
      ((free ++ linUnless (not b1) locked) ++ linUnless (not b2) (linUnless b1 locked))
      (const ())) ->
  Parser state tok b2 (linUnless b1 locked) (free ++ linUnless (not b1) locked) a ->
  Parser state tok (b1 && b2 && b3 && True) locked free a
enclose left right p = (\[_, x, _] => x) <$> Seq [left, p, right]

%inline
public export
option : Parser state tok False locked free a -> Parser state tok True locked free (Maybe . a)
...
plus :
  ...
  Parser state tok False locked free a ->
  Parser state tok False locked free (List1 . a)
plus p =
  Fix "plus"
    (   uncurry (:::)
...
star :
  {auto len : LengthOf locked} ->
  Parser state tok False locked free a ->
  Parser state tok True locked free (List . a)
star p = maybe [] forget <$> option (plus p)

public export
sepBy1 :
  {auto len : LengthOf locked} ->
  (sep : Parser state tok False locked free (const ())) ->
  Parser state tok False locked free a ->
  Parser state tok False locked free (List1 . a)
